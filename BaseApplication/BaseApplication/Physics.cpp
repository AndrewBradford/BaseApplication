...
void Physics::make_test_graph_branch(glm::vec3 dir, float z_offset, float stretch)
{
...
void Physics::make_test_graph_big_branch(glm::vec3 dir, float z_offset, float stretch)
{
...
void Physics::resolve_all_constraints()
{
	int counter = 0;
...
void Physics::particle_update(float speed, float dt)
{
	// for each particle
	for (int i = 0; i < space_graph.size(); i++)
	{
...
void Physics::get_data(float& line, float& grad, int& unin, int& sequ)
{
